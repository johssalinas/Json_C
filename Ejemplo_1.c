...
JSONArray parse_array(JSONParser *parser) {
    ...
    while (parser->current_token.type != TOKEN_ARRAY_END) {
        ...
        array.elements[array.size] = malloc((array.size+1)*sizeof(ListObjects*));
        array.elements[array.size]->head = malloc((array.size+1)*sizeof(ListObjects*));
        //array.elements[array.size]->head[i] = malloc(sizeof(JSONObject*));
        switch (parser->current_token.type) {
            case TOKEN_STRING:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 1;
                array.elements[array.size]->head->value.string_value = strdup(parser->current_token.value);
                break;
            case TOKEN_NUMBER:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 2;
                array.elements[array.size]->head->value.number_value = atof(parser->current_token.value);
                break;
            case TOKEN_OBJECT_BEGIN:
                array.elements[array.size]->head = parse_object(parser, listObjects)->head;
                break;
            case TOKEN_ARRAY_BEGIN:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 4;
                array.elements[array.size]->head->value.array_value = parse_array(parser);
                break;
            case TOKEN_TRUE:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 5;
                array.elements[array.size]->head->value.bool_value = 1;
                break;
            case TOKEN_FALSE:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 5;
                array.elements[array.size]->head->value.bool_value = 0;
                break;
            case TOKEN_NULL:
                array.elements[array.size]->head->key = NULL;
                array.elements[array.size]->head->value_type = 6;
                break;
            default:
// error
                return array;
